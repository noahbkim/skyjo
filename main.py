...
class RandomPlayer(skyjo.Player):
    ...
    def turn(self, state: skyjo.State, action: skyjo.Turn) -> None:
        ...
        if choice == 1:
            action.draw_card()
            action.place_drawn_card(
                self.hand.find_first_unflipped_or_highest_card_index()
            )
        elif choice == 2:
            action.place_from_discard(
                self.hand.find_first_unflipped_or_highest_card_index()
            )
        elif choice == 3:
            action.draw_card()
...
def main() -> None:
    ...
    parser.add_argument(
        "players",
        nargs="+",
        help="qualified player classes to instantiate.",
        metavar="PLAYER",
    )
    parser.add_argument(
        "-n", "--games", type=int, default=1, help="the number of games to play."
    )
    parser.add_argument(
        "-s",
        "--seed",
        type=int,
        default=None,
        help="a seed for random number generation.",
    )
    ...
    parser.add_argument(
        "-r", "--repro", default=None, help="A repro code to debug a broken rule."
    )
