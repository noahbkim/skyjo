...
Skyjo: TypeAlias = tuple[Game, Table, Deck, int]
...
def new(players: int) -> Skyjo:
    ...
    deck[:] = CARD_COUNTS
    return game, table, deck, players
...
def get_second_flip_mask(skyjo: Skyjo) -> np.ndarray[tuple[int], np.uint8]:
    """Compute the space of options for a second flip.

    At the start of the game before a discard is dealt the (0, 0) card
    is flipped for every player in a vacuum (they cannot observe each
    other). They are then given the choice of whether to flip the card
    in the same column, [1, 0], or in the same row [0, 1]. Therefore,
    this mask is a constant [1, 1].
    """

    return np.array([1, 1], dtype=np.uint8)  # type: ignore


def get_draw_or_take_mask(skyjo: Skyjo) -> np.ndarray[tuple[int], np.uint8]:
    """The space of options for turn actions.

    At the start of each turn, players must either draw a card from the
    pile, [1, 0], or take the discard, [0, 1]. Therefore, this mask is
    a constant [1, 1].
    """
...
def get_flip_or_replace_mask_for_draw(
    skyjo: Skyjo,
) -> np.ndarray[tuple[int, int, int], np.uint8]:
    """The space of options if the user draws a card.

    If the player draws a card, they must either discard it and flip
    a card in their hand, [0, row, column], or replace a card in their
    hand with it, discarding what it replaced, [1, row, column].
    """

    table = skyjo[1]

    result = np.ndarray((2, 3, 3), dtype=np.uint8)
    for row in range(ROW_COUNT):
        for column in range(COLUMN_COUNT):
            if table[0, row, column, FINGER_HIDDEN]:
                result[:, row, column] = 1  # You both flip or replace
            elif not table[0, row, column, FINGER_CLEARED]:
                result[1, row, column] = 1  # You can only replace

    return result


def get_flip_or_replace_mask_for_take(
    skyjo: Skyjo,
) -> np.ndarray[tuple[int, int, int], np.uint8]:
    """The space of options if the user takes the discard.

    If the player takes the discard, they must replace a card in their
    hand with it, discarding what it replaced, [1, row, column].
    """
...
def deal(skyjo: Skyjo, card: int | None = None, rng: Random = random) -> Skyjo:
    ...
    return new_game, table, new_deck, players
...
def flip(
    ...
    return new_game, new_table, new_deck, players
